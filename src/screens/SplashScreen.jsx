    ...
    return (
        <View style={tw`h-full w-full`}>
            ...
            <ImageBackground blurRadius={7} 
                source={{ uri: `http://118.70.118.186:8070/web/api/v1/get_background_app?image_type=img_app&model=dm.diadiem&time=${Math.random()}` }} 
                resizeMode="cover" style={tw`h-full w-full flex items-center justify-center `}>
                    <Image 
                        source={{ uri: `http://118.70.118.186:8070/web/api/v1/get_background_app?image_type=logo&model=dm.diadiem&time=${Math.random()}`}}
                        resizeMode="contain"
