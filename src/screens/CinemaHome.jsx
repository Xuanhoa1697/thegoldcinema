    ...
    const get_list_cinema_home = async () => {
        try {
            ...
            let config = {
                ...
                mode: 'no-cors',
                url: `https://thegoldcinema.com/web/api/v1/get_list_cinema_home`,
                headers: {
            ...
            <TouchableOpacity style={tw`w-full border-b border-[#c9c9c9]`} onPress={() => navigation.goBack()}>
                {datas.length > 0 && datas.map((item, index) => {
                    return (<View key={index} style={tw`h-[75px] w-full flex-row items-center justify-between px-3`}>
                        <Text style={tw`text-[12px] font-semibold text-black`}>{item.name}</Text>
                        <Text ellipsizeMode='tail' numberOfLines={2} style={tw`text-[12px] text-[#9c1d21] w-[250px]`}>{item.address}</Text>
                    </View>)
                })}
            </TouchableOpacity>
        </View>
