        ...
        return (
            <View style={tw``}>
                <View style={tw`w-full`}>
                    <Text style={tw`text-[13.5px] text-[#404040]`}>{item.dm_loaighe_name}</Text>
                </View>
            </View>
        )
    }
        ...
        <View style={tw`h-full w-full`}>
            ...
            <View style={tw`h-[65px] w-full flex-row items-center justify-start px-2 border-b border-gray-300 absolute z-1 bg-white`}>
                ...
                <View style={tw`flex items-start justify-center ml-2`}>
                    ...
                    <View style={tw`flex-row items-start justify-center`}>
                        <Text numberOfLines={1} ellipsizeMode='tail' style={tw`text-[13.5px] text-[#9c9c9c]`}>{detail.phong},</Text>
                        <Text numberOfLines={1} ellipsizeMode='tail' style={tw`text-[13.5px] text-[#9c9c9c] ml-1`}>{detail.ngaychieu.split("-")[2]}-{detail.ngaychieu.split("-")[1]}-{detail.ngaychieu.split("-")[0]},</Text>
                        <Text numberOfLines={1} ellipsizeMode='tail' style={tw`text-[13.5px] text-[#9c9c9c] ml-1`}>{detail.giobatdau} ~ {detail.ketthuc}</Text>
                    </View>
                </View>
                ...
                <Modal
                    ...
                    <View style={tw`flex-1 justify-center items-center p-3`}>
                        <View style={tw`bg-white w-full pt-4 pb-6  px-6 rounded-2 bg-[#ffffff] border border-gray-300`}>
                            ...
                            <Text style={tw`text-[13.5px] text-[#000000] mt-3`}>{datas.ptthanhtoan.length > 0 ? datas.ptthanhtoan[0].luu_y : 'Vui lòng kiểm tra trước khi thanh toán.'}</Text>
                            ...
                            <View key={item} style={tw`w-full h-[65px] flex-row items-center justify-between px-3 mt-2 bg-[#ffffff] rounded`}>
                                <View style={tw`flex-row items-center justify-between`}>
                                    ...
                                    <SelectCountry
                                        ...
                                        selectedTextStyle={tw`text-[13.5px] text-[#404040]`}
                                        placeholderStyle={tw`text-[13.5px]`}
                                ...
                                <Text style={tw`text-[#9C1D21] font-bold text-[13.5px]`}>{(selectedBanggia[item] || defaultDongia).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} đ</Text>
                            </View>
                        )
                    })}
                </ScrollView>
            ...
            <View style={tw`h-[90px] w-full flex-row items-center justify-between px-2 absolute z-1 bottom-0 bg-white`}>
                <View style={tw`flex items-start justify-start w-full`}>
                    ...
                    <View style={tw`flex-row items-center justify-between w-full mt-2`}>
                        <Text style={tw`text-[#9C1D21] font-bold text-[13.5px]`}>{Object.values(selectedBanggia).reduce((a, b) => a + b, 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} đ</Text>
                        <TouchableOpacity onPress={showModal}>
                            <Text style={tw`text-white bg-[#9C1D21] text-[13.5px] px-8 py-2 rounded-15`}>Thanh toán</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </View>
